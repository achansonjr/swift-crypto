...
func randomSerialNumber() -> ASN1_INTEGER {
    ...
    let bn = CCryptoBoringSSL_BN_new()
    defer {
        CCryptoBoringSSL_BN_free(bn)
    }
    
    _ = readBytes.withUnsafeBufferPointer {
        CCryptoBoringSSL_BN_bin2bn($0.baseAddress, $0.count, bn)
    }

    // We want to bitshift this right by 1 bit to ensure it's smaller than
    // 2^159.
    CCryptoBoringSSL_BN_rshift1(bn, bn)

    // Now we can turn this into our ASN1_INTEGER.
    var asn1int = ASN1_INTEGER()
    CCryptoBoringSSL_BN_to_ASN1_INTEGER(bn, &asn1int)
...
func generateRSAPrivateKey() -> UnsafeMutablePointer<EVP_PKEY> {
    ...
    defer {
        CCryptoBoringSSL_BN_free(exponent)
    }

    CCryptoBoringSSL_BN_set_u64(exponent, 0x10001)

    let rsa = CCryptoBoringSSL_RSA_new()!
    let generateRC = CCryptoBoringSSL_RSA_generate_key_ex(rsa, CInt(2048), exponent, nil)
    precondition(generateRC == 1)

    let pkey = CCryptoBoringSSL_EVP_PKEY_new()!
    let assignRC = CCryptoBoringSSL_EVP_PKEY_assign(pkey, EVP_PKEY_RSA, rsa)
...
func addExtension(x509: UnsafeMutablePointer<X509>, nid: CInt, value: String) {
    ...
    CCryptoBoringSSL_X509V3_set_ctx(&extensionContext, x509, x509, nil, nil, 0)
    let ext = value.withCString { (pointer) in
        return CCryptoBoringSSL_X509V3_EXT_nconf_nid(nil, &extensionContext, nid, UnsafeMutablePointer(mutating: pointer))
    }!
    CCryptoBoringSSL_X509_add_ext(x509, ext, -1)
    CCryptoBoringSSL_X509_EXTENSION_free(ext)
}
