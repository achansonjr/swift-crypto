...
public class PrivateKey {
    ...
    private convenience init(file: String, format: CryptoSerializationFormats, callbackManager: CallbackManagerProtocol?) throws {
        ...
        let key = withExtendedLifetime(callbackManager) { callbackManager -> UnsafeMutablePointer<EVP_PKEY>? in
            guard let bio = CCryptoBoringSSL_BIO_new_fp(fileObject, BIO_NOCLOSE) else {
                return nil
            }
            defer {
            ...
            case .pem:
                ...
                if let callbackManager = callbackManager {
                    return CCryptoBoringSSL_PEM_read_PrivateKey(fileObject, nil, { globalBoringSSLPassphraseCallback(buf: $0, size: $1, rwflag: $2, u: $3) }, Unmanaged.passUnretained(callbackManager as AnyObject).toOpaque())
                } else {
                    return CCryptoBoringSSL_PEM_read_PrivateKey(fileObject, nil, nil, nil)
                }
            case .der:
                return CCryptoBoringSSL_d2i_PrivateKey_fp(fileObject, nil)
            }
        }

        if key == nil {
            throw CryptoCertificateError.failedToLoadPrivateKey
        }
    ...
    private convenience init(bytes: [UInt8], format: CryptoSerializationFormats, callbackManager: CallbackManagerProtocol?) throws {
            ...
            return withExtendedLifetime(callbackManager) { callbackManager -> UnsafeMutablePointer<EVP_PKEY>? in
                ...
                case .pem:
                    if let callbackManager = callbackManager {
                        // This annoying conditional binding is used to work around the fact that I cannot pass
                        // a variable to a function pointer argument.
                        return CCryptoBoringSSL_PEM_read_bio_PrivateKey(bio, nil, { globalBoringSSLPassphraseCallback(buf: $0, size: $1, rwflag: $2, u: $3) }, Unmanaged.passUnretained(callbackManager as AnyObject).toOpaque())
                    } else {
                        return CCryptoBoringSSL_PEM_read_bio_PrivateKey(bio, nil, nil, nil)
                    }
                case .der:
                    return CCryptoBoringSSL_d2i_PrivateKey_bio(bio, nil)
                }
            }
        }
    ...
    ///         encrypted keys.
    public convenience init<T: Collection>(file: String, format: CryptoSerializationFormats, passphraseCallback: @escaping NIOSSLPassphraseCallback<T>) throws where T.Element == UInt8 {
    ...
    public convenience init<T: Collection>(buffer: [Int8], format: CryptoSerializationFormats, passphraseCallback: @escaping NIOSSLPassphraseCallback<T>) throws where T.Element == UInt8  {
    ...
    ///         behaviour will be used, which prints a prompt and requests the passphrase from
    ///         stdin.
    public convenience init<T: Collection>(bytes: [UInt8], format: CryptoSerializationFormats, passphraseCallback: @escaping NIOSSLPassphraseCallback<T>) throws where T.Element == UInt8  {
        let manager = BoringSSLPassphraseCallbackManager(userCallback: passphraseCallback)
    ...
    static internal func fromUnsafePointer(takingOwnership pointer: UnsafeMutablePointer<EVP_PKEY>) -> PrivateKey {
        return PrivateKey(withReference: pointer)
    }
...
extension PrivateKey {
    ...
    private func withUnsafeDERBuffer<T>(_ body: (UnsafeRawBufferPointer) throws -> T) throws -> T {
        ...
        let rc = CCryptoBoringSSL_i2d_PrivateKey_bio(bio, self.ref)
        guard rc == 1 else {
...
extension PrivateKey: Equatable {
    public static func ==(lhs: PrivateKey, rhs: PrivateKey) -> Bool {
        ...
        return CCryptoBoringSSL_EVP_PKEY_cmp(lhs.ref, rhs.ref) == 1
    }
}
